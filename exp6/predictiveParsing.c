...
void followfirst(char , int , int); void findfirst(char , int , int); void follow(char c); 
 
int count,n=0; char calc_first[10][100]; char calc_follow[10][100]; int m=0; 
...
{  
    ...
    for(k=0;k<count;k++) 
    { 
        c=production[k][0];  point2 = 0;  xxx = 0; 
    ...
    for(e=0;e<count;e++) 
    { 
        ck=production[e][0];  point2 = 0;  xxx = 0; 
        for(kay = 0; kay <= ptr; kay++)  	
            if(ck == donee[kay]) 
                xxx = 1; 
        if (xxx == 1) 
    ...
    for(ap=0;ap<count;ap++){  
        int destiny = 0;  k = 2; 
    int ct = 0;  char tem[100];  while(production[ap][k] != '\0'){ 
        ...
        else{  	 	int zap=0;  	 	int tuna = 0;  	 	for(zap=0;zap<count;zap++){ 
    ...
    int zap = 0,tuna;  for(tuna = 0;tuna<ct;tuna++){ 
        if(tem[tuna] == '#'){ 
            zap = 1; 
        } 
        else if(tem[tuna] == '_'){ 
    ...
    char table[land][sid+1];  ptr = -1; 
    for(ap = 0; ap < land ; ap++){  for(kay = 0; kay < (sid + 1) ; kay++){ 
        table[ap][kay] = '!'; 
    }  } 
    for(ap = 0; ap < count ; ap++){  
        ck = production[ap][0];  
    ...
    for(ap = 0; ap < count ; ap++){  
        ...
        if(ni == 1){  	 	char xz = production[ap][0]; 
            int cz=0; 
            while(table[cz][0] != xz){ 
                cz = cz + 1; 
            } 
    ...
    for(k=0;k<sid;k++){  for(kay=0;kay<100;kay++){ 
        if(calc_first[k][kay] == '!'){ 
            break; 
        } 
    ...
    int vz=0; 
    while(ter[vz] != calc_follow[k][fz]){ vz = vz + 1; 
    } table[k][vz+1] = '#'; 
            fz++; 
            } 
            break; 
    ...
    char input[100];  scanf("%s%c",input,&ch); 
    printf("\n\t\t\t\t\t===========================================================================\n"); 
    ...
    printf("\t\t\t");  vamp = i_ptr;  while(input[vamp] != '\0'){ 
        printf("%c",input[vamp]); 
        vamp++; 
    } 
    printf("\t\t\t");  char her = input[i_ptr];  char him = stack[s_ptr]; 
    s_ptr--; 
    if(!isupper(him)){ 
        if(her == him){  	 	i_ptr++; 
            printf("POP ACTION\n"); 
        ...
        int le = strlen(produ);  	le = le - 1;  	if(le == 0){ 
            continue; 
        } 
        for(j=le;j>=2;j--){ 
            s_ptr++; 
    ...
    printf("\n\t\t\t================================= ======================================================================================\n"); 
    if (input[i_ptr] == '\0'){  printf("\t\t\t\t\t\t\t\tYOUR STRING HAS BEEN ACCEPTED !!\n"); 
    } 
